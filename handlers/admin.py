...
async def _legacy_export(message: Message) -> None:
    ...
    await message.answer(
        ...
        "<code>Здравствуйте! 🧡 Пишу вам лично от сообщества «Подслушано в Нидерландах».\n\n"
        "Вы — один из специалистов нашего гайда (он есть и на сайте, и в нашем "
        "Telegram-боте), и были с нами с самого начала, когда размещение было "
        "бесплатным и бессрочным. Спасибо вам за доверие!\n\n"
        "Хочу честно и заранее предупредить: чтобы гайд жил, регулярно обновлялся и "
        "продвигался (а вас находило больше людей), мы переводим размещение на "
        f"платную основу с {deadline:%d.%m.%Y}.\n\n"
        "Для вас — как для тех, кто с нами с самого начала, — мы оставили особую "
        f"цену: {y_price} {cur} в год вместо обычных {norm_price} {cur}.\n\n"
        "Чтобы ваша карточка осталась в гайде, оплатите по личной ссылке до "
        f"{deadline:%d.%m.%Y}:\nССЫЛКА\n\n"
        "Если продолжать не хотите — ничего делать не нужно, карточка просто "
        "перестанет показываться, без каких-либо обязательств. Но мы будем очень "
        "рады, если останетесь с нами 🧡</code>",
        reply_markup=main_menu(),
    )
