...
ALLO_INTRO = (
    ...
    "🌟 Allo Walks — только начало: скоро откроем закрытый клуб Allo. Абонемент "
    "сейчас зачтётся в будущее членство клуба (подробнее — в самом абонементе).\n\n"
    "Выбирай 👇"
)
...
def _pass_card() -> str:
    return (f"🎟 <b>Абонемент Allo Walks</b>\n\n"
            ...
            "🌟 <b>Это только начало.</b> Allo Walks перерастёт в закрытый клуб Allo — "
            "больше, чем прогулки. Поэтому первые 2 месяца действует особый бонус: "
            f"<b>€{_p(config.ALLO_PRICE_PASS)} за абонемент зачтём в будущее членство "
            "клуба «Всё включено».</b> Возьмёшь абонемент сейчас — считай, уже вложился "
            "в клуб.\n\n"
