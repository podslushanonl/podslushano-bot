...
# Фразы-просьбы совета (не поиск специалиста): «что делать», «как быть»…
_ADVICE_PHRASES = (
    "что делать", "что мне делать", "как быть", "как мне быть", "что грозит",
    "что будет если", "что будет, если", "куда обращаться", "куда идти",
    "как поступить", "имею ли право", "что посоветуете", "посоветуйте что",
    "нужен совет", "нужен ваш совет", "что делать в", "что делать,",
)
# Прямой отказ от поиска специалиста — отвечаем советом (ИИ), а не ищем.
_NEG_SEARCH = ("не ищу", "не нужен", "не нужна", "не надо", "не нужно")


def is_howto_question(text: str) -> bool:
    """True, если это вопрос-инструкция/просьба совета, а не поиск специалиста.

    Ловим: «как <сделать что-то>», «что делать / как быть / что грозит…», и
    прямой отказ «не ищу/не нужен …». Обычные «нужен juрист в Амстердаме» —
    остаются поиском по гайду."""
    low = (text or "").lower()
    if re.search(r"\bкак\b", low) and any(v in low for v in _HOWTO_VERBS):
        return True
    if any(p in low for p in _ADVICE_PHRASES):
        return True
    if any(p in low for p in _NEG_SEARCH):
        return True
    return False
...
_INFO_TOPICS = (
    ...
    "налог", "декларац", "aangifte", "belasting", "штраф", "boete", "пенси",
)
