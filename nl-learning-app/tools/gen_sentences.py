...
ADJ_SKIP={'злой','сердитый','больной','женатый','голодный','усталый','сытый','живой','старый','молодой','мёртвый','радостный','испуганный','довольный','весёлый','грустный','счастливый'}
NEED_SKIP={'страна','сторона','провинция','север','юг','запад','восток','остров','деревня','город','лес','море','пляж','природа','погода','солнце','ветер','дождь','снег','облако','тело','лицо','семья','человек','люди'}
...
def frames_for(theme_i,kind,disp,ru,lemma):
    ...
    if kind=='obj':
        ...
        if theme_i==3 and head not in DRINKS:  # еда
            add(cap(D)+' is lekker.',cap(P['n'])+' — '+gsel(g,'вкусный','вкусная','вкусное','вкусные')+'.')
...
json.dump({'note':'Батч №1 предложений A1 (тематические шаблоны + русская морфология). Источник уроков и пула экзаменов.','count':len(items),'items':items},
 open(ROOT+'/data/sentences_batch1.json','w',encoding='utf-8'),ensure_ascii=False,indent=1)
# выборка на ревью
import random
random.seed(7)
