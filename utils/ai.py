...
async def extract_specialist_query(
    ...
    system = (
        ...
        "категорий списка. Автомастерская, автомойка, детейлинг, шиномонтаж, "
        "химчистка салона и ремонт автомобиля относятся к категории «автосервис». "
        "Уборка дома и мойка окон относятся к категории «клининг».\n"
        "- Если ищут услугу/профессию, которой НЕТ в списке (например: грумер, "
        "эвакуатор или аренда авто) — category=null. НЕ подгоняй под похожую "
        "категорию.\n"
...
_INTENT_SYSTEM = (
    ...
    "«ищу автомастерскую в Гааге», «где сделать детейлинг автомобиля».\n"
    ...
    "«не ищу бухгалтера, нужен совет», «объясни про toeslag», «найди интересные "
    "места в Гааге», «посоветуй кафе или музей в Амстердаме».\n"
...
_HUMAN_STYLE = (
    "\n\nСТИЛЬ ПИСЬМА (обязательно для всех русских текстов — заголовков, текста "
    "слайдов, подписи, поста):\n"
    "- простые слова и короткие предложения; пиши естественно, по-человечески, "
    "как живой автор; можно начинать предложения с «и», «но», «так»;\n"
    "- прямо и лаконично: убирай лишние слова и повторы, добивайся ясности;\n"
    "- искренний человеческий тон, без наигранной дружелюбности и без роботических "
    "ноток; текст не должен выглядеть машинным;\n"
    "НЕ используй:\n"
    "- рекламные и маркетинговые фразы;\n"
    "- штампы вроде «погрузимся», «раскроем потенциал», «отправимся в путешествие», "
    "«давайте посмотрим»;\n"
    "- вводные слова «в общем», «очевидно», «интересно»;\n"
    "- риторические вопросы;\n"
    "- конструкции «X, а также Y»;\n"
    "- тире «—» (вообще не используй; разделяй мысли точками и запятыми) и лишние двоеточия.\n"
    "Это НЕ относится к обязательным служебным элементам (призыв подписаться со "
    "ссылкой, строка про 🔥, CTA-слайд) — их оставляй как задано выше."
)
...
_POST_SYSTEM = (
    ...
    "куда-то ещё.\n\n"
    "Тон и стиль:\n"
    "- ЖИВОЙ редакционный язык от лица команды podslushano.nl — как люди, которые "
    "здесь живут и делятся наблюдениями: с характером, тёплый, но без фамильярности, "
    "сленга и грубостей. НЕ сухая канцелярщина;\n"
    "- чётко, структурно, без воды; конкретика: факты, цифры, источники;\n"
    ...
    "Формат:\n"
    "- цепляющий ЖИВОЙ заголовок ПЕРВОЙ строкой в <b>…</b>;\n"
    "- 2–3 смысловых блока; подзаголовки в <b>…</b> конкретные и с характером. "
    "НЕ используй шаблонные/кликбейтные подзаголовки вроде «Что происходит», "
    "«Программа для всех», «Смотри фото» — пиши по сути и по-человечески;\n"
    "- списки через «• …»; умеренно эмодзи (по смыслу, не больше одного на блок);\n"
    "- короткий практичный вывод/совет;\n"
    "- В САМОМ КОНЦЕ поста, отдельными строками, ровно два элемента ПО ПОРЯДКУ:\n"
    "  1) ровно такой строкой: «Если пост оказался полезен — ставьте 🔥, чтобы "
    "больше таких анонсов появлялось в ленте.»;\n"
    "  2) живой призыв подписаться, АДАПТИРОВАННЫЙ под тему поста, со ссылкой на "
    "канал в слове «Подписывайтесь»: "
    '<a href="https://t.me/podslushanovnl">Подписывайтесь</a>, чтобы … '
    "(продолжение по смыслу темы, в духе «быть в курсе того, что реально "
    "происходит в стране»).\n\n"
    ...
    "Технически: по-русски. Разметка ТОЛЬКО тегами <b>, <i> и <a href> (для Telegram); "
    "никаких #, *, markdown и других тегов. Внешние ссылки на источники — обычным "
    "текстом (https://…); тег <a> используй для ссылки на наш канал в призыве "
    "подписаться. Пост идёт С ФОТО, поэтому уложись в ~900 символов (это подпись к фото; "
    "ссылка <a> в лимит подписи не входит).\n\n"
...
async def pick_best_photo(description: str, urls: list[str]) -> str | None:
    ...
    content: list[dict] = [{
        ...
        "text": (
            ...
            f"место/объект/сюжет из Нидерландов по теме.\n"
            f"Требования к выбору:\n"
            f"- предпочитай эффектный ракурс (вид с высоты/дрон, красивый свет, "
            f"необычная перспектива), а не банальный кадр перед зданием;\n"
            f"- предпочитай вертикальные кадры с главным объектом ближе к центру "
            f"(их меньше обрезает формат 4:5);\n"
            f"- предпочитай виды мест, природы и архитектуры. НЕ выбирай фото, где "
            f"крупным планом люди как главный объект; и не выбирай кадры, где в "
            f"центре внимания темнокожие люди.\n"
            f"Ответь ТОЛЬКО числом — номером фото (1–{len(urls)}); ответь 0, если "
            f"ни одно не подходит."
        ),
    }]
...
async def ai_channel_post(
    ...
    Если переданы base_text и instruction — не генерим заново, а ПРАВИМ
    существующий пост по указанию редактора."""
    if not ai_enabled() or not (topic or "").strip():
        return None
    if base_text and instruction:
        content = (
            f"Текущий пост:\n{base_text}\n\n"
            f"Внеси правки и верни ОБНОВЛЁННЫЙ пост целиком по тем же правилам "
            f"(включая концовку с призывом подписаться и строкой про 🔥, и строку "
            f"IMG_QUERY). Правки от редактора: {instruction.strip()}"
        )
    else:
        content = f"Тема поста: {topic.strip()}"
    messages = [{"role": "user", "content": content}]
    client = _get_client()
    try:
...
_ARTICLE_SYSTEM = (
    "Ты — редактор медиа о Нидерландах (podslushano.nl) для русскоязычной "
    "аудитории. Пишешь увлекательные и содержательные статьи о стране: "
    "ПУТЕШЕСТВИЯ и места (города, регионы, маршруты, природа, что посмотреть и "
    "попробовать), ИСТОРИЯ, КУЛЬТУРА и искусство, СПОРТ, МЕДИА и новости, "
    "события и традиции. Это не справка по бюрократии — это живой, интересный "
    "рассказ.\n\n"
    "Требования к содержанию:\n"
    "• Пиши по-русски, живо и увлекательно, но по делу — без воды, канцелярита "
    "и общих фраз. Сложное объясняй простым языком.\n"
    "• Мы НЕ пересказываем — мы рассказываем интересно: своя подача, любопытные "
    "детали, контекст, короткие истории, наблюдения и подмечания.\n"
    "• Раскрывай тему насыщенно и конкретно: названия мест и городов, факты, "
    "даты, имена, цифры, детали, практичные советы для путешественника (как "
    "добраться, когда ехать, что рядом, цены/часы, если знаешь).\n"
    "• Где это реально помогает читателю — вставляй ССЫЛКИ прямо в текст HTML-"
    "тегом <a href=\"URL\">текст</a>: официальный сайт места/музея/события, "
    "билеты, карта, расписание. Ставь только РЕАЛЬНЫЕ ссылки, которые нашёл или "
    "знаешь точно; не выдумывай URL. Не пихай ссылки ради галочки.\n"
    "• Если статья — список (напр. «7 городов»), делай КАЖДЫЙ пункт отдельным "
    "разделом с подзаголовком <h2> (по одному <h2> на пункт) — так к каждому "
    "можно прикрепить своё фото.\n"
    "• Структурируй: цепляющее вступление (1–2 абзаца), затем разделы с "
    "подзаголовками <h2>, в конце — короткий вывод.\n\n"
    "Формат ответа СТРОГО такой:\n"
    "Первая строка — заголовок статьи обычным текстом, без разметки и без слова "
    "«Заголовок».\n"
    "Дальше пустая строка, затем тело в ПРОСТОМ HTML: абзацы <p>, подзаголовки "
    "<h2> (и при необходимости <h3>), списки <ul><li>/<ol><li>, выделение "
    "<strong>. Абзацы короткие. Без <h1>, без <html>/<body>, без markdown, без "
    "``` и без картинок.\n\n"
    "Объём — 700–1200 слов. Не выдумывай факты, даты и имена. Если не уверен в "
    "факте — проверь поиском или не пиши его. По событиям/новостям и точным "
    "датам сверяйся с актуальными источниками."
)
...
async def ai_site_article(topic: str) -> tuple[str, str] | None:
    ...
    client = _get_client()
    try:
        kwargs = dict(model=config.AI_POST_MODEL, max_tokens=3600,
                      system=_ARTICLE_SYSTEM, messages=messages)
        # Для travel/культуры/истории/спорта нужен широкий поиск, не только госсайты
        ...
        try:
            resp = await client.messages.create(
                model=config.AI_POST_MODEL, max_tokens=3600,
                system=_ARTICLE_SYSTEM, messages=messages,
            )
        except Exception as e2:  # noqa: BLE001
...
_IG_SYSTEM = (
    ...
    '"body": "1-3 предложения с КОНКРЕТИКОЙ: точные названия, цифры, цены, '
    'адреса, как добраться, часы работы (до 200 знаков)", '
    ...
    "}\n\n"
    "Правила:\n"
    "- carousel: 1 обложка (headline) + 5–7 содержательных слайдов в массиве slides. "
    "Финальный CTA-слайд добавлять НЕ нужно — бот сделает его сам. Для короткой "
    "новости/одного факта используй type=single, slides=[].\n"
    "- ВЕСЬ текст — обычный, БЕЗ HTML, markdown, символов #, * и звёздочек "
    "(он ляжет прямо на картинку и в подпись).\n"
    "- ГРАМОТНОСТЬ КРИТИЧНА: пиши на безупречном русском — правильные падежи, "
    "согласование родов и чисел, без опечаток и кривых формулировок. Перечитай "
    "текст перед выдачей и исправь любые ошибки.\n"
    "- НЕ используй тире «—» вообще (ни в заголовках, ни в тексте). Строй короткие "
    "простые предложения, разделяй их точками и запятыми.\n"
    ...
    "- каждый слайд — про ОДИН пункт/мысль, с ДЕТАЛЯМИ и конкретикой (факты, "
    "цифры, цены в €, адреса, как добраться, часы, нюансы), а НЕ общие слова. "
    "Используй веб-поиск, чтобы детали были настоящими;\n"
    "- ПРИЗЫВЫ (сохранить пост, подписаться, написать в комментариях) — только в "
    "caption. В обычных слайдах (slides) призывов быть НЕ должно. НЕ делай "
    "отдельный слайд-призыв и НЕ заканчивай последний слайд прощанием — финальный "
    "CTA-слайд добавляется автоматически отдельно;\n"
    "- img_query — КОНКРЕТНЫЙ визуальный запрос на английском про ИМЕННО это место "
    "из слайда, передающий его суть, чтобы захотелось поехать. Указывай название "
    "места + город + Netherlands и самый красивый, «открыточный» ракурс (общий план, "
    "пейзаж, золотой час), напр. 'Kasteel de Haar castle Utrecht Netherlands scenic', "
    "'Biesbosch national park canoe aerial Netherlands'. БЕЗ людей крупным планом;\n"
    ...
    "- НЕ описывай свои действия и процесс — верни только JSON." + _HUMAN_STYLE
)
...
async def ai_instagram_carousel(topic: str, base_json: str | None = None,
                                instruction: str | None = None) -> dict | None:
    """Контент для Instagram-карусели по теме. Возвращает dict со структурой
    _IG_SYSTEM (type/headline/cover_img_query/slides/caption/hashtag) или None.
    Фото НЕ подбирает — только текст и англ. запросы (img_query) для фото.

    Если переданы base_json и instruction — правит существующую карусель."""
    if not ai_enabled() or not (topic or "").strip():
        return None
    if base_json and instruction:
        user = (
            f"Текущая карусель (JSON):\n{base_json}\n\n"
            f"Внеси правки и верни ОБНОВЛЁННЫЙ JSON в той же структуре и по тем же "
            f"правилам. Правки от редактора: {instruction.strip()}"
        )
    else:
        user = f"Тема поста: {topic.strip()}"
    ...
    # Instagram-карусель: максимум 10 фото (обложка + слайды + CTA) → контента ≤ 8
