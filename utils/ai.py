...
async def pick_best_photo(description: str, urls: list[str]) -> str | None:
    ...
    content: list[dict] = [{
        ...
        "text": (
            ...
            f"место/объект/сюжет из Нидерландов по теме. При прочих равных "
            f"предпочитай эффектный, интересный ракурс (вид с высоты/дрон, "
            f"необычная перспектива, красивый свет), а не банальный кадр перед "
            f"зданием. Ответь ТОЛЬКО числом — номером фото (1–{len(urls)}); "
            f"ответь 0, если ни одно не подходит."
        ),
    }]
