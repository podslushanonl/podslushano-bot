...
def _strip_model_preamble(text: str) -> str:
    """Remove planning/reasoning accidentally emitted before the publishable post."""
    clean = (text or "").strip()
    ...
    leaked = (
        ...
        "это последний шанс", "создаёт реальный дедлайн",
    )
...
async def _rich_event() -> str | None:
    ...
    system = (
        "Ты редактор русскоязычного Telegram-медиа о Нидерландах. Найди ОДНО действительно интересное "
        "реальное мероприятие на ближайшие 14 дней. В приоритете evenementen.nl, затем официальный сайт "
        "события или городская афиша. Проверь дату, место, цену и актуальный статус. "
        "Верни ТОЛЬКО готовый текст публикации. Никогда не пиши, как ты выбирал тему, почему она сильная, "
        "что материал найден, какой у него дедлайн, и не добавляй служебный анализ до поста. "
        "Пост должен быть живым и содержательным: цепляющий заголовок, затем 3-5 абзацев с историей, "
        "неожиданной деталью или контекстом, из-за которого событие хочется увидеть. Не пересказывай афишу сухо. "
        "В конце обязательно практический блок: дата, место/город, цена или условия входа, официальный источник/ссылка. "
        "1100-1700 знаков. Без markdown, без рекламных клише и без фраз вроде «уникальная возможность» или «не пропустите»."
    )
    result = await editorial._generate(
        system,
        f"Сегодня {editorial._now():%d.%m.%Y}. Не повторяй: {', '.join(recent) or 'нет'}. Верни только публикацию.",
        editorial.EVENT_SOURCES,
        1400,
    )
    if not result:
        return None
...
def install_caption_patch() -> None:
    ...
    editorial._event_spotlight = _rich_event
    editorial._curiosity_post = _rich_curiosity
