...
def _normalize_morning_output(text: str) -> str:
    """Keep only the publishable morning post and normalize spacing."""
    clean = _clean_text(text)
    start = clean.find("Доброе утро!")
    if start >= 0:
        clean = clean[start:]
    clean = re.sub(
        r"\n\nНравится разбор с утра\? Ставьте 🔥\s*$",
        "",
        clean,
        flags=re.IGNORECASE,
    )
...
def _morning_quality_errors(text: str) -> list[str]:
    ...
    forbidden = (
        ...
        "вот готовый", "результат поиска", "критично!", "max ", "min ",
        "прошедшей ночью",
    )
...
async def _morning_brief() -> str | None:
    system = (
        "Ты выпускающий редактор утреннего Telegram-поста Podslushano.nl для людей, которые "
        "живут в Нидерландах. Нужен готовый человеческий пост уровня ежедневного медиа, а не "
        ...
        "Ориентир по тону: «Доброе утро! Сегодня погоду затмевает другая новость — ...». "
        ...
        "Закончить практическим советом.\n\n"
    ...
    for attempt in range(2):
        ...
        if last_errors:
            correction = (
                "\n\nПредыдущий черновик отклонён редактором: "
                + "; ".join(last_errors)
                + ". Перепроверь источники и перепиши пост полностью."
            )
        result = await _generate(
            system,
            base_request + correction,
            MORNING_SOURCES,
            2200,
        )
        if not result:
            continue
        text = _normalize_morning_output(result[0])
