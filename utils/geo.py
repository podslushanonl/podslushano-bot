...
CATEGORIES: dict[str, list[str]] = {
    ...
    "музыкант и диджей": ["музыкант", "живая музыка", "кавер-групп", "кавер групп",
                           "диджей", "dj", "вокалист на праздник", "певец на праздник",
                           "певица на праздник"],
    "организация мероприятий": ["организатор мероприят", "организация мероприят",
                                 "event agency", "ивент-агент", "ивент агент",
                                 "свадебн организатор", "координатор свадьб"],
    ...
    "продукты и магазины": ["магазин продукт", "продуктов магазин", "продажа продукт",
                            "икра", "деликатес", "лавка", "магазин русск",
                            "магазин украинск", "продукты с доставк"],
    # --- 🏠 Дом и быт ---
    "ремонт": ["ремонт квартир", "ремонт дом", "ремонт ванн", "строительн", "плотник",
                "отделочн", "отделк", "малярн", "плитк", "штукатур", "покрасочн работ",
                "напольн покрыт", "bouw", "verbouw"],
    "мастер на час": ["электрик", "сантехник", "газов", "котл", "klusjes", "handyman",
                       "тёплый пол", "теплый пол", "отоплен", "сборка мебел", "мастер на час"],
    # Автомобильные услуги идут раньше клининга: «химчистка салона» — это
    # детейлинг автомобиля, а не уборка дома.
    "автосервис": [
        "автосервис", "автомастер", "автомойк", "шиномонтаж", "ремонт авто",
        "garage", "автомеханик", "автоэлектрик", "детейлинг", "детелинг",
        "detailing", "полировк кузов", "химчистк салон", "тонировк авто",
        "тонировк окон автомобил", "ppf", "керамическ покрыт кузов",
        "реставрац фар", "брендирован автомобил",
    ],
    ...
    "языковые курсы": ["английск", "нидерландск", "голландск", "немецк", "испанск",
                        ...
                        "школа язык", "языков"],
    "музыкальные занятия": ["урок вокал", "преподавател вокал", "занятия вокал",
                             "урок фортепиано", "урок пианино", "музыкальн школ",
                             "обучение музык", "обучение вокал", "сольфеджио",
                             "урок гитар", "урок скрипк", "хор для"],
...
THEMES: dict[str, list[str]] = {
    ...
    "🏠 Дом и быт": ["ремонт", "мастер на час", "клининг", "переезды", "риелтор"],
    "🚗 Авто": ["автосервис", "автошкола"],
    "👶 Дети и образование": ["репетитор", "языковые курсы", "музыкальные занятия",
                               "няня", "детские занятия"],
    "🍽 Еда и продукты": ["кондитер", "кейтеринг", "продукты и магазины"],
    "🎉 События и контент": ["фотограф", "видеограф", "ведущий", "музыкант и диджей",
                             "организация мероприятий", "декор", "аниматор"],
    "💼 Профессиональные услуги": ["юрист", "бухгалтер", "переводчик", "it и веб",
                                   "маркетинг", "дизайнер", "бизнес-консалтинг",
...
# Дополнительные направления разрешаем только для осмысленных сочетаний.
# Иначе любое случайное слово в описании даёт мусор: нутрициолог, написавший
# «после переезда», не должен появляться в категории грузоперевозок.
_SECONDARY_CATEGORY_COMPATIBILITY: dict[str, set[str]] = {
    "дизайнер": {"ремонт", "творчество"},
    "психолог": {"коуч", "карьерный консультант"},
    "коуч": {"парикмахер"},
    "парикмахер": {"стилист"},
    "перманентный макияж": {"брови и ресницы", "визажист"},
    "косметолог": {"эпиляция"},
    "эпиляция": {"косметолог"},
    "бизнес-консалтинг": {"маркетинг", "карьерный консультант"},
    "it и веб": {"маркетинг"},
    "музыкальные занятия": {"репетитор"},
    "фотограф": {"видеограф"},
}
...
def _keyword_matches(text: str, keyword: str) -> bool:
    """Сопоставляет слово/корень, не цепляясь за середину другого слова.

    Раньше ``"тату"`` находилось внутри ``"ситуация"``, а короткое ``"spa"`` —
    внутри названия ``"Space"``. Для русских корней достаточно границы слева,
    а короткие латинские слова должны совпадать целиком.
    """
    ...
    for part in parts:
        ...
        if part.isascii() and part.isalnum() and len(part) <= 3:
            patterns.append(rf"(?<![\w]){escaped}(?![\w])")
        else:
            # Категорийные фразы хранятся как корни: «химчистк салон»
            # должно находить «химчистка салона».
            patterns.append(rf"(?<![\w]){escaped}\w*")
...
def detect_categories(text: str) -> list[str]:
    """Возвращает все подходящие категории в порядке приоритета."""
    return [
        category
...
def detect_category(text: str) -> str | None:
    """Определяет главную категорию специалиста по тексту запроса (или None)."""
    matches = detect_categories(text)
    return matches[0] if matches else None


def specialist_matches_category(specialist, category: str) -> bool:
    """Карточка относится к категории явно или по описанию услуг.

    Это позволяет находить одну карточку по нескольким релевантным направлениям
    без дублирования: например, «фотограф / видеограф» показывается в обеих
    категориях, а языковая школа — и по названию конкретного языка.
    """
    if specialist.category == category:
        return True
    if category not in CATEGORIES:
        return False
    if category not in _SECONDARY_CATEGORY_COMPATIBILITY.get(specialist.category, set()):
        return False
    text = f"{specialist.name} {specialist.description or ''}"
    return any(_keyword_matches(text, keyword) for keyword in CATEGORIES[category])
