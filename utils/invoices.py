"""Генерация счёта (factuur), постоянный архив и отправка на e-mail.

Цена считается ВКЛЮЧАЮЩЕЙ BTW (config.BTW_PERCENT). Номера счетов — сквозные
(хранятся в таблице Meta). Каждый PDF сначала сохраняется на постоянный Railway
volume, затем отправляется клиенту. Копия также уходит на COMPANY_EMAIL, поэтому
доступ к старым фактурам больше не зависит от срока хранения Resend.
"""
...
def invoice_archive_root() -> Path:
    """Постоянное хранилище фактур рядом с bot.db на Railway volume."""
    return Path(config.DB_PATH).resolve().parent / "invoices"


def invoice_archive_path(no: str, invoice_date: date) -> Path:
    """Путь вида data/invoices/2026/08/factuur-2026-0028.pdf."""
    return (
...
def _archive_pdf(no: str, invoice_date: date, pdf: bytes) -> Path:
    """Атомарно сохраняет PDF до попытки отправки e-mail."""
...
def _build_pdf(
    ...
    *,
    ...
    for line in [
        ...
        f"KvK: {config.COMPANY_KVK}  ·  BTW: {config.COMPANY_BTW}",
    ...
    y -= 5 * mm
    c.drawString(x, y, f"Datum / Дата: {invoice_date:%d-%m-%Y}")
...
def archive_invoice_copy(
    no: str,
    invoice_date: date,
    buyer_name: str,
    description: str,
    total_str: str,
    buyer_lines: list | None = None,
) -> Path:
    """Восстанавливает/архивирует фактуру с уже существующим номером.

    Не меняет invoice_seq и ничего не отправляет клиенту. Нужен для безопасного
    восстановления исторических PDF, которые уже были выданы покупателям.
    """
    ...
    return _archive_pdf(no, invoice_date, pdf)


async def _send_email(
    to: str, subject: str, html: str, pdf: bytes, filename: str
) -> tuple[bool, str]:
    body = {
    ...
    # Постоянная копия в нашей Gmail. Даже когда Resend удалит запись из
    # журнала, полученное письмо и PDF останутся у Podslushano.nl.
    own_email = (config.COMPANY_EMAIL or "").strip()
    if own_email and own_email.lower() != to.strip().lower():
        body["bcc"] = [own_email]
    headers = {
    ...
    class _SMTP(smtplib.SMTP_SSL):
        def _get_socket(self, h, p, t):  # noqa: ANN001
            ipv4 = socket.getaddrinfo(
                h, p, socket.AF_INET, socket.SOCK_STREAM
            )[0][4][0]
...
async def _send_gmail(
    ...
    try:
        await asyncio.to_thread(
            _send_smtp_sync, to, subject, html, pdf, filename
        )
        return True, ""
    except Exception as e:  # noqa: BLE001
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status >= 300:
                        detail = (
                            f"Resend HTTP {response.status}: {await response.text()}"
                        )
                        log.warning(detail)
                        return False, detail
                    return True, ""
...
async def send_invoice(
    ...
    """Создаёт, архивирует и отправляет PDF-фактуру.

    Архивирование выполняется ДО внешней отправки: временный сбой Resend больше
    не приводит к потере бухгалтерского экземпляра.
    """
    if not config.invoice_enabled():
        return False, "не настроен ни Gmail (GMAIL_APP_PASSWORD), ни Resend"
    if not to_email:
    ...
    pdf = _build_pdf(
        no,
        buyer_lines or [buyer_name, to_email],
        description,
        excl,
        btw,
        total,
    ...
    try:
        archive_path = _archive_pdf(no, invoice_date, pdf)
        log.info("Фактура %s сохранена: %s", no, archive_path)
    except Exception as exc:  # noqa: BLE001
        # Не отправляем единственный экземпляр наружу, если локальный архив не
        # создался: иначе снова получим ситуацию, когда у клиента PDF есть, а у нас нет.
        detail = f"не удалось сохранить фактуру {no}: {type(exc).__name__}: {exc}"
        log.error(detail)
        return False, detail
